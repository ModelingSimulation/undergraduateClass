...
\text{such that }&H({\bf X}) = 0.
\end{align*} 
Now, try to argue geometrically that we can pick ${\bf X}$ so that $H({\bf X}) = 0$ and $({\bf X} - {\bf X}^0) \cdot {\bf V}_i = 0$ for $i = 2, 3$.  In fact, we can pick ${\bf X}$ so that ${\bf X} - {\bf X}^0$ is parallel to ${\bf V}_1$. 
